...
export class OwnerdetailsComponent implements OnInit {
  ...
  // ✅ Called when food type radio button changes
    ...
    if (input.files && input.files.length > 0) {
      ...
      files.forEach((file) => {
        ...
        this.cloudServ.uploadImages(file).subscribe({
          next: (res: any) => {
            const imageUrl = res;
            const messImagesArray = this.myMessForm.get('messDetails.messImages') as FormArray;
  ...
  expandAll() { this.menuList.forEach((item) => (item.expanded = true)); }
  collapseAll() { this.menuList.forEach((item) => (item.expanded = false)); }
  ...
  onMessDetailsSubmit() {
    ...
    this.messDetailsServ.postMessOwnerDetailsList(this.myMessForm.value).subscribe({
      next: (response: any) => {
  ...
  removeLicenseImage(): void {
    ...
    const input = document.getElementById('licenseFileInput') as HTMLInputElement;
