...
export class CustomerComponent {
  ...
  openMyFilter(): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        this.applyFilters(); // Call the combined filter method
      }
    });
  }
  ...
  getCityName(myNewMessDetailsInformationObj: any) {
    ...
    myNewMessDetailsInformationObj.messDetails.forEach((mess: any) => {
      const city = mess.address.city.trim();
      if (city && !this.locations.some(loc =>
        loc.toLowerCase() === city.toLowerCase()
      )) {
        this.locations.push(city);
      }
    });
    // Initialize filteredCities with all locations
  ...
  navigateToShowMessInfo(id: any) {
    console.log(id);
    this.router.navigate(['/messinfo/', id], { relativeTo: this.activeRoute });
