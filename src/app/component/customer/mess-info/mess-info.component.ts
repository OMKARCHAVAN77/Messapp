...
export class MessInfoComponent {
  ...
  myRatingData: any[] = [];
  timeInfo:any;
  constructor(private customerServ: CustomerService, private activeRoute: ActivatedRoute, public dialog: MatDialog, private cdRef: ChangeDetectorRef) { }
  ...
  this.myNewMessDetailsInformationObj.timeDetails = data.timeDetails; // <-- required
}
    });
 ...
 getRelatedDetails(userId: any): any {
  ...
  openMyFeedBackBox() {
    ...
    // Add this new method to refresh rating data
    // refreshRatingData() {
    //   this.customerServ.getCustomerInMessDetails().subscribe((data: any) => {
    //     console.log('Full response data:', data); // Debug the full response
    ...
    //       }
    //     } else {
    //       console.log('No rating data found in response');
    //     }
    //   }, (error) => {
